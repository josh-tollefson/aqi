...
processed = processed[processed['O3'].notna() & processed['PM10'].notna() & processed['NO2'].notna()]
...
if button:
	### FORECAST AQI
	# import sys
	# sys.path.insert(1, os.getcwd() + '../forecasting') # how to make this compatible w/ windows & osx?
	import forecast

	# Filter data to desired zipcode
	...
	# import pickle
	# f = pickle.load(open('arima_O3_PM10_NO2_zipcode-93062.sav','rb'))

	# Run Forecast
	nobs = 7
	f.run_forecast(nobs = nobs)

	predictions = pd.DataFrame(f.predictions, columns = VARIABLES, index = np.arange(1, nobs + 1))
	predictions
