...
class Channel():
    ...
    # updated by aaron in Oct 2020!
    def get_historical(self,
                       weeks_to_get: int,
                       thingspeak_field: str,
                       average=60, # average=x (get average of x minutes, valid values: 10, 15, 20, 30, 60, 240, 720, 1440)
                       ) -> pd.DataFrame:
        """
        Get data from the ThingSpeak API one week at a time up to weeks_to_get weeks in the past
        """
        print('hello from aaron 1')
        if average not in (10, 15, 20, 30, 60, 240, 720, 1440):
            raise Exception('Average does have valid value.')
        ...
        # pylint: disable=line-too-long
        url = f'https://thingspeak.com/channels/{channel}/feed.csv?api_key={key}&offset=0&average={average}&round=2&start={to_week.strftime("%Y-%m-%d")}%2000:00:00&end={from_week.strftime("%Y-%m-%d")}%2000:00:00'
        print(url)
    ...
    def get_daily(self,
            ...
            if delta.days > 8000:
                ...
                from_week = to_week - delta8000

            # pylint: disable=line-too-long
            url = f'https://thingspeak.com/channels/{channel}/feed.csv?api_key={key}&offset=0&average=1440&round=2&start={to_week.strftime("%Y-%m-%d")}%2000:00:00&end={from_week.strftime("%Y-%m-%d")}%2000:00:00'
            if print_url:
                print(url)
            weekly_data = pd.read_csv(url)
            # if weeks_to_get > 1:
